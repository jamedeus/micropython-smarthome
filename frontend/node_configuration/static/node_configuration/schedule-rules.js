function add(button) {
    ...
    // Add timestamp field
    cell_time.innerHTML = `<input type='text' class='form-control ${instance} timestamp' id='schedule-${instance}-rule${next_row}-time' placeholder='HH:MM' name='schedule-${instance}-rule${next_row}-time'>`;
    ...
    if (instance.startsWith("device") && document.getElementById(instance.replace("device", "deviceType")).value == "api-target") {
        cell_value.innerHTML = `<button id="schedule-${instance}-rule${next_row}-button" class="form-control" onclick="open_rule_modal(this);" type="button">Set rule</button>
                                <input type="text" class="form-control ${instance} rule" id="schedule-${instance}-rule${next_row}-value" placeholder="" name="schedule-${instance}-rule${next_row}-value" style="display:none;">`

    // For all other instance types, add input field
    } else {
        cell_value.innerHTML = `<input type='text' class='form-control ${instance} rule' id='schedule-${instance}-rule${next_row}-value' placeholder='' name='schedule-${instance}-rule${next_row}-value'>`;
    };
