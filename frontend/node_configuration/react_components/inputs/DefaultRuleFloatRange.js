...
function DefaultRuleFloatRange({ id, value, metadata, onChange }) {
    return (
        <InputWrapper label="Default Rule">
            <div className="d-flex flex-row align-items-center my-2">
                ...
                <input type="range" className="mx-auto" min={metadata.rule_limits[0]} max={metadata.rule_limits[1]} data-displaymin={metadata.rule_limits[0]} data-displaymax={metadata.rule_limits[1]} data-displaytype="float" step="0.5" value={value} onChange={(e) => onChange(id, e.target.value)} autoComplete="off" />
