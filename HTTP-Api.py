# This is an unused, fully HTTP-compatible version of Api.py
# Latency is approximately doubled compared to the normal version, but it can be used in a web browser
# It is unlikely that this can be optimized further due to a bandwidth bottleneck caused by the
# ESP32's low powered antennas - the only thing that works is reducing payload size (breaking HTTP)
#
# In the future, I may consider running both APIs on seperate ports (untested, may impact performance)
...
class Api:
    ...
    async def run_client(self, sreader, swriter):
        try:
            # Read client request
            request_line = await asyncio.wait_for(sreader.readline(), self.timeout)
