...
class TestWled(unittest.TestCase):
    ...
    def __dir__(self):
        return ["test_instantiation", "test_rule_validation_valid", "test_rule_validation_invalid", "test_rule_change", "test_enable_disable", "test_disable_by_rule_change", "test_enable_by_rule_change", "test_turn_off", "test_turn_on", "test_regression_invalid_default_rule"]
    ...
    def test_regression_invalid_default_rule(self):
        ...
        try:
            test = Wled("device1", "device1", "dimmer", True, None, "disabled", "192.168.1.211")
        ...
        try:
            test = Wled("device1", "device1", "dimmer", True, None, "enabled", "192.168.1.211")
            # Should not make it to this line, test failed
            self.assertFalse(True)
        except AttributeError:
            # Should raise exception, test passed
