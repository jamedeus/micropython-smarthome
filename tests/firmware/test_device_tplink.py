...
class TestTplink(unittest.TestCase):
    ...
    @cpython_only
    def test_08_check_brightness(self):
        from unittest.mock import patch

        # Simulate dimmer status object with brightness = 100
        self.instance._type = 'dimmer'
    ...
    def test_17_monitor(self):
        ...
        # Set current_rule to 50
        self.instance.current_rule = 50

        # Run loop for 1 second while mocking status response to brightness=75
        with patch.object(self.instance, '_check_brightness', return_value=75):
            asyncio.run(test_monitor_and_kill())

        # Confirm current_rule changed to 75
